...
```{r}
present_old <- old_names[old_names %in% names(df2)]
present_new <- new_names[old_names %in% names(df2)]
```
...
```{r}
df2_clean$sexe <- recode_factor(df2_clean$sexe,
                               "1" = "Homme",
                               "2" = "Femme")
```

```{r}
df2_clean$interet_info <- recode_factor(
  df2_clean$interet_info,
  "1" = "Très intéressé",
  "2" = "Assez intéressé",
  "3" = "Peu intéressé",
...
```{r}
df2_clean$info_fiable <- recode_factor(
  df2_clean$info_fiable,
  "1" = "Très facile",
  "2" = "Plutôt facile",
  "3" = "Plutôt difficile",
  "4" = "Très difficile"
...
```{r}
df2_clean$opinion_sujet <- recode_factor(
  df2_clean$opinion_sujet,
...
```{r}
df2_clean$situa_pro <- recode_factor(
  df2_clean$situa_pro,
...
```{r}
df2_clean$niveau_etude <- recode_factor(
  df2_clean$niveau_etude,
  `1` = "Niveau primaire",
